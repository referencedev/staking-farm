...
fn store_contract() {
    // TODO: Why do we need this low level implementation? Is the performance
    // TODO: Given we are using low level implementation can we wrap all sys::* functions
    //       inside other functions with small unsafe blocks such as done in env on near-sdk-rs.
    //       In this way big chunks of unsafe code is no longer required,
    //       where other code that is not mean't to be unsafe slip over.
    unsafe {
        ...
        //       needs to be brute-forced to generate the key collision.
